...
from NiimPrintX.nimmy.printer import MAX_RASTER_WIDTH, InfoEnum, PrinterClient, RequestCodeEnum
...
def test_print_image_runs_complete_protocol_sequence(monkeypatch):
    ...
    monkeypatch.setattr("NiimPrintX.nimmy.printer.asyncio.sleep", AsyncMock())
...
def test_print_image_times_out_and_terminates_started_job(monkeypatch):
    ...
    printer.end_print = AsyncMock(return_value=True)
    monkeypatch.setattr("NiimPrintX.nimmy.printer.PRINT_POLL_INTERVAL_SECONDS", 0.01)
